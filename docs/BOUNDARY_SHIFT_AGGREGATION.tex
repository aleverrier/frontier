...
threshold: from 5235 to 2346 for X and from 4914 to 2318 for Z.  It remains
insufficient at \(K=1024\) and \(K=2048\).  Counting higher polymers cannot
repair this.  The next audit should inspect compatibility conflicts at the
cuts dominating the grouped \(K=1024\) expression, to decide between exact
component/treewidth elimination and direct dominant-comparison-spectrum
calculation.
...
\end{verbatim}
The two-scope run took approximately \(3.51\) seconds on one CPU.  Its
SHA256 is
\begin{verbatim}
d4a5dc3a7d3aed11fe1e8e26e41cde7ed037e9e6ee7c0850ad0dabe9de08ccfb
\end{verbatim}
Regression tests compare (8) with brute subset enumeration and verify (4)
cut by cut.  No decoder or Monte Carlo sampling is used.
